...
def train_predict_adaboost(normalized_features, feature_selector, y, num_runs=20, cv=False):
    '''
        ...
        if not cv:
            ...
            train = np.concatenate(
                (cv_indices[0:i * fold_length],
                 cv_indices[
                    (i + 1) * fold_length:normalized_features.shape[0]]))
        predictions, model = azimuth_adaboost(
            None, train, test, y, None, selected_features,
            None, None, learn_options, False)
